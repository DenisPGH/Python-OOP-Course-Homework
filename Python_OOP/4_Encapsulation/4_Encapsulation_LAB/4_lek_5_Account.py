...
class Account:
    ...
    def get_id(self, pin): # if the given pin is correct, return the id, otherwise, return "Wrong pin"
        if not pin ==self.__pin:
            return "Wrong pin"
        return self.__id
    def change_pin(self,old_pin, new_pin): # - if the old pin is correct, change it to the new one and return "Pin changed", otherwise return "Wrong pin"
