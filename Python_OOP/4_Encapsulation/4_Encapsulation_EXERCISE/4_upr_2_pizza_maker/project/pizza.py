...
•	name: str - if the name is an empty string, raise a ValueError with the
 message "The name cannot be an empty string"
•	dough: Dough - if the dough is None, raise a ValueError with the
message "You should add dough to the pizza"
•	toppings_capacity: int – represents the maximum number of toppings the pizza should have.
If the capacity is 0 or less, raise a ValueError with the message
"The topping's capacity cannot be less or equal to zero"
•	toppings: dict – empty dictionary upon initialization that will contain the topping type
 as a key and the topping's weight as a value.
"""
class Pizza:
    ...
    # o	If there is no space left for a new topping, raise a
    ...
    # (dough's weight and toppings' weight)
    ...
    def calculate_total_weight(self):
        result_weight=0
        for each_topping in self.toppings:
            result_weight+=self.toppings[each_topping]
        result_weight+=self.dough.weight
