...
from test.astronaut.astronaut_repository import AstronautRepository
from test.astronaut.biologist import Biologist
from test.astronaut.geodesist import Geodesist
from test.astronaut.meteorologist import Meteorologist
from test.planet.planet import Planet
from test.planet.planet_repository import PlanetRepository
...
"""
    ...
    def add_astronaut(self,astronaut_type:str, name: str):
        ...
        #person_to_add=Geodesist(name)
        #person_to_add=Biologist(name)
        #person_to_add=Meteorologist(name)
        #person_to_add=astronaut_type(name)
...
"""
        ...
        for current_astronaut in list_going_out:
            if len(target_planet.items)==0:
                ...
                return f"Planet: {planet_name} was explored. {len(final_astr_list)} astronauts participated in collecting items."
    ...
    def report(self):
        ...
        for each_ast in self.astronaut_repository.astronauts:
            result+=f"Name: {each_ast.name}\n" \
                    ...
                    f"Backpack items: {each_ast.backpack}\n"
